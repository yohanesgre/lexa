...
beforeAll(async () => {
  dir = mkdtempSync(join(tmpdir(), "lexa-forge-sessions-"));
...
describe("PUT /api/forge/sessions", () => {
  it("upserts a mapping with 204 and no body", async () => {
    const res = await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody() }));
    expect(res.status).toBe(204);
    expect(await res.text()).toBe("");
  });

  it("upserting the same ref twice rewrites the row (204 both times)", async () => {
    const res1 = await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody() }));
    expect(res1.status).toBe(204);
    const res2 = await handler(adminReq("/api/forge/sessions", {
      method: "PUT",
      body: upsertBody({ runtimeSessionId: "sess-2", skillId: "sk2" }),
    }));
    expect(res2.status).toBe(204);
    const list = await handler(adminReq("/api/forge/sessions?documentType=task&documentId=t1"));
...
describe("GET /api/forge/sessions", () => {
  it("returns the mapping as data (camelCase, updatedAt set)", async () => {
    await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody() }));
    const res = await handler(adminReq("/api/forge/sessions?documentType=task&documentId=t1"));
  ...
  it("is scoped to the document — other documents get an empty list", async () => {
    await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody() }));
    const res = await handler(adminReq("/api/forge/sessions?documentType=wiki&documentId=w1"));
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("missing query params are an empty list, not an error", async () => {
    const res = await handler(adminReq("/api/forge/sessions"));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.data).toEqual([]);
  });
...
describe("DELETE /api/forge/sessions", () => {
  it("removes the mapping with 204", async () => {
    await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody() }));
    const res = await handler(adminReq("/api/forge/sessions", { method: "DELETE", body: JSON.stringify({ documentType: "task", documentId: "t1", runtimeId: "rt1" }) }));
    expect(res.status).toBe(204);
    const list = await handler(adminReq("/api/forge/sessions?documentType=task&documentId=t1"));
    const body = await list.json();
    expect(body.data).toEqual([]);
  ...
  it("does NOT 409 when a task is running for the document (daemon-side drop)", async () => {
    ...
    const body = await list.json();
    expect(body.data).toEqual([]);
    db.prepare("DELETE FROM forge_tasks WHERE id = 'ft1'").run();
  });

  it("deleting a missing mapping is 204, not 404", async () => {
    const res = await handler(adminReq("/api/forge/sessions", { method: "DELETE", body: JSON.stringify({ documentType: "task", documentId: "ghost", runtimeId: "rt1" }) }));
    expect(res.status).toBe(204);
  });
});

describe("POST /api/forge/sessions/reset", () => {
  it("deletes the mapping with 204", async () => {
    await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody() }));
    const res = await handler(adminReq("/api/forge/sessions/reset", { method: "POST", body: JSON.stringify({ documentType: "task", documentId: "t1", runtimeId: "rt1" }) }));
    expect(res.status).toBe(204);
    const list = await handler(adminReq("/api/forge/sessions?documentType=task&documentId=t1"));
    const body = await list.json();
    expect(body.data).toEqual([]);
  ...
  it("resets only the given runtime's mapping", async () => {
    await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody({ runtimeId: "rt1" }) }));
    await handler(adminReq("/api/forge/sessions", { method: "PUT", body: upsertBody({ runtimeId: "rt2" }) }));
    const res = await handler(adminReq("/api/forge/sessions/reset", { method: "POST", body: JSON.stringify({ documentType: "task", documentId: "t1", runtimeId: "rt1" }) }));
    expect(res.status).toBe(204);
    const list = await handler(adminReq("/api/forge/sessions?documentType=task&documentId=t1"));
    const body = await list.json();
  ...
  it("409 FORGE_SESSION_ACTIVE when a queued task exists for the document+runtime", async () => {
    db.prepare(
      `INSERT INTO forge_tasks (id, project_id, document_type, document_id, agent_id, skill_id, status, runtime_id, created_at)
    ...
    const body = await res.json();
    expect(body.error.code).toBe("FORGE_SESSION_ACTIVE");
    db.prepare("DELETE FROM forge_tasks WHERE id = 'ft2'").run();
  });
  ...
  it("409 when a running task exists for the document+runtime", async () => {
    ...
    const res = await handler(adminReq("/api/forge/sessions/reset", { method: "POST", body: JSON.stringify({ documentType: "task", documentId: "t1", runtimeId: "rt1" }) }));
    expect(res.status).toBe(409);
    db.prepare("DELETE FROM forge_tasks WHERE id = 'ft3'").run();
  });
  ...
  it("204 when the active task is on a different runtime (per-runtime reset)", async () => {
    ...
    const res = await handler(adminReq("/api/forge/sessions/reset", { method: "POST", body: JSON.stringify({ documentType: "task", documentId: "t1", runtimeId: "rt1" }) }));
    expect(res.status).toBe(204);
    db.prepare("DELETE FROM forge_tasks WHERE id = 'ft4'").run();
  });

  it("no 404s: resetting a missing mapping is 204", async () => {
    const res = await handler(adminReq("/api/forge/sessions/reset", { method: "POST", body: JSON.stringify({ documentType: "task", documentId: "ghost", runtimeId: "rt1" }) }));
    expect(res.status).toBe(204);
  });
});
