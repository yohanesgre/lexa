...
beforeAll(async () => {
  dir = mkdtempSync(join(tmpdir(), "lexa-assistant-probe-http-"));
...
describe("POST /api/admin/assistant/providers/:id/probe", () => {
  it("success closes breaker and returns row", async () => {
    ...
    const res = await handler(authed("POST", "/api/admin/assistant/providers/pr1/probe"));
  ...
  it("failed probe on open breaker stays open with bumped counts", async () => {
    db.prepare("UPDATE assistant_provider_health SET failure_count = 3, circuit_state = 'open', opened_at = ?, consecutive_failures = 3 WHERE provider_id = 'pr1'").run(new Date().toISOString());
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("down")));
    const res = await handler(authed("POST", "/api/admin/assistant/providers/pr1/probe"));
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("unknown provider → 404", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(okModels));
    const res = await handler(authed("POST", "/api/admin/assistant/providers/nope/probe"));
  ...
  it("non-admin → 403", async () => {
    ...
    const res = await handler(new Request("http://lexa.test/api/admin/assistant/providers/pr1/probe", { method: "POST", headers: { authorization: `Bearer ${memberKey}` } }));
