...
beforeAll(async () => {
  ...
  dir = mkdtempSync(join(tmpdir(), "lexa-assistant-chat-upgrades-"));
...
INSERT INTO assistant_threads (document_type, document_id, project_id, owner_user_id, title, messages)
VALUES ('chat', 'chat-err', 'p2', 'u1', 'Broken turn', '[{"role":"user","content":"hi"},{"role":"assistant","content":"","error":{"code":"ASSISTANT_GENERATION_FAILED","message":"boom"}}]');
-- Export fixture with per-entry meta.
INSERT INTO assistant_threads (document_type, document_id, project_id, owner_user_id, title, messages)
VALUES ('chat', 'chat-x', 'p1', 'u1', NULL, '[{"role":"user","content":"What is Lexa?","ts":"2026-08-22T10:00:00.000Z"},{"role":"assistant","content":"A project tool.","ts":"2026-08-22T10:00:05.000Z","citations":[{"title":"Lexa Docs","url":"https://lexa.example/guide"},{"title":null,"url":"https://raw.example/x"}]},{"role":"user","content":"continue"},{"role":"assistant","content":"par","error":{"code":"ASSISTANT_GENERATION_FAILED","message":"boom"}}]');
-- List fixtures: pinned beats recency.
INSERT INTO assistant_threads (document_type, document_id, project_id, owner_user_id, title, pinned, messages, updated_at)
VALUES ('chat', 'chat-pin', 'p1', 'u1', 'Old but pinned', 1, '[]', datetime('now', '-3 hours'));
INSERT INTO assistant_threads (document_type, document_id, project_id, owner_user_id, title, messages, updated_at)
...
describe("GET /api/assistant/chats/:projectId (q + pinned ordering)", () => {
  it("pins first, then recency; other users invisible", async () => {
    const res = await handler(authed("GET", "/api/assistant/chats/p1"));
    ...
    const annas = await handler(authed("GET", "/api/assistant/chats/p1", undefined, SECOND_ADMIN_KEY));
  ...
  it("?q= matches transcript text and returns a snippet window", async () => {
    const res = await handler(authed("GET", "/api/assistant/chats/p1?q=needle"));
    const body = await res.json();
  ...
  it("?q= title-only match → snippet null", async () => {
    const res = await handler(authed("GET", "/api/assistant/chats/p1?q=Recency"));
    const body = await res.json();
...
describe("PATCH /api/assistant/chat/:chatId (meta)", () => {
  it("pins and unpins; persists", async () => {
    const pin = await handler(authed("PATCH", "/api/assistant/chat/chat-rec", { pinned: true }));
    expect(pin.status).toBe(200);
    expect(await pin.json()).toEqual({ chatId: "chat-rec", title: "Recency", pinned: true });
    const row = db.prepare(`SELECT pinned FROM assistant_threads WHERE document_id = 'chat-rec'`).get() as { pinned: number };
    expect(row.pinned).toBe(1);
    const unpin = await handler(authed("PATCH", "/api/assistant/chat/chat-rec", { pinned: false }));
    expect((await unpin.json()).pinned).toBe(false);
  });

  it("updates title and pinned together", async () => {
    const res = await handler(authed("PATCH", "/api/assistant/chat/chat-rec", { title: "  Renamed Rec  ", pinned: true }));
    expect(res.status).toBe(200);
  ...
  it("empty payload or blank/over-long title → 422 INVALID_ARGS", async () => {
    for (const payload of [{}, { title: "   " }, { title: "x".repeat(201) }]) {
      const res = await handler(authed("PATCH", "/api/assistant/chat/chat-rec", payload));
...
describe("GET /api/assistant/chat/:chatId/export", () => {
  it("renders markdown attachment with template markers; untitled → chat-<date>.md", async () => {
    const res = await handler(authed("GET", "/api/assistant/chat/chat-x/export"));
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toContain("text/markdown");
    const day = db.prepare(`SELECT updated_at FROM assistant_threads WHERE document_id = 'chat-x'`).get() as { updated_at: string };
    ...
    expect(md).toContain("**Assistant** · 2026-08-22T10:00:05.000Z\nA project tool.");
    expect(md).toContain("- [Lexa Docs](https://lexa.example/guide)");
    expect(md).toContain("- <https://raw.example/x>");
    expect(md).toContain("[failed turn: ASSISTANT_GENERATION_FAILED]");
  });

  it("titled thread sanitizes into the filename and header", async () => {
    await handler(authed("PATCH", "/api/assistant/chat/chat-x", { title: "My Chat!" }));
    const res = await handler(authed("GET", "/api/assistant/chat/chat-x/export"));
  ...
  it("another user's thread → 404 ASSISTANT_THREAD_NOT_FOUND", async () => {
    const res = await handler(authed("GET", "/api/assistant/chat/chat-x/export", undefined, SECOND_ADMIN_KEY));
...
describe("POST /api/assistant/chat/stream (fromIndex)", () => {
  it("edit flow: truncates to fromIndex, appends the new turn, persists ts meta", async () => {
    const put = await handler(
      authed("PUT", "/api/assistant/settings/p2", {
  ...
  it("bad fromIndex → 422 INVALID_ARGS, transcript untouched", async () => {
    for (const fromIndex of [99, 1, 1.5]) {
      const res = await handler(
        authed("POST", "/api/assistant/chat/stream", { projectId: "p2", chatId: "chat-edit", message: "nope", fromIndex })
      );
      expect(res.status).toBe(422);
      expect((await res.json()).error.code).toBe("INVALID_ARGS");
    }
    const row = db.prepare(`SELECT messages FROM assistant_threads WHERE document_id = 'chat-edit'`).get() as { messages: string };
  ...
  it("second concurrent stream on an active chat → 409 ASSISTANT_TASK_ACTIVE", async () => {
    ...
    const first = await handler(
      new Request("http://lexa.test/api/assistant/chat/stream", {
        method: "POST",
    ...
    const second = await handler(
      authed("POST", "/api/assistant/chat/stream", { projectId: "p1", chatId: "c-live", message: "again" })
    );
    expect(second.status).toBe(409);
    expect((await second.json()).error.code).toBe("ASSISTANT_TASK_ACTIVE");
    ...
    const third = await handler(
      authed("POST", "/api/assistant/chat/stream", { projectId: "p1", chatId: "c-live", message: "fresh" })
    );
