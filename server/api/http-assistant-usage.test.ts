...
beforeAll(async () => {
  dir = mkdtempSync(join(tmpdir(), "lexa-assistant-usage-http-"));
  ...
  `);
  db.exec(`
    INSERT INTO assistant_call_logs (id, project_id, provider_id, model, kind, status, usage_in, usage_out, cached_in, latency_ms, cost_cents, created_at)
    VALUES
...
describe("GET /api/admin/assistant/usage", () => {
  ...
  it("filters by from/to inclusive", async () => {
    const res = await handler(authed("GET", "/api/admin/assistant/usage?from=2026-08-02&to=2026-08-02"));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it("filters by projectId", async () => {
    const res = await handler(authed("GET", "/api/admin/assistant/usage?projectId=p1"));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it("computes nearest-rank p50/p95 over non-null latencies (odd count)", async () => {
    db.exec(`
      ...
      INSERT INTO assistant_call_logs (id, project_id, provider_id, model, kind, status, usage_in, usage_out, cached_in, latency_ms, cost_cents, created_at) VALUES
    ...
    `);
    const res = await handler(authed("GET", "/api/admin/assistant/usage?projectId=p3"));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.summary.totalCalls).toBe(4);
  ...
  it("returns null p50/p95 when no latencies are recorded", async () => {
    ...
    const res = await handler(authed("GET", "/api/admin/assistant/usage?projectId=p4"));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it("non-admin → 403", async () => {
    ...
    const res = await handler(new Request("http://lexa.test/api/admin/assistant/usage", { headers: { authorization: `Bearer ${memberKey}` } }));
    expect(res.status).toBe(403);
  });
});

describe("GET /api/admin/assistant/usage.csv", () => {
  it("returns csv with header and rows", async () => {
    const res = await handler(authed("GET", "/api/admin/assistant/usage.csv"));
    expect(res.status).toBe(200);
  ...
  it("filters csv by projectId", async () => {
    const res = await handler(authed("GET", "/api/admin/assistant/usage.csv?projectId=p2"));
    const text = await res.text();
  ...
  it("non-admin → 403 csv", async () => {
    const memberKey = "lxk_" + "m".repeat(43);
    const res = await handler(new Request("http://lexa.test/api/admin/assistant/usage.csv", { headers: { authorization: `Bearer ${memberKey}` } }));
    expect(res.status).toBe(403);
  });
});

describe("PUT /api/admin/assistant/prices", () => {
  it("upserts price and returns row", async () => {
    const res = await handler(authed("PUT", "/api/admin/assistant/prices", { model: "anthropic/claude-sonnet-4", prompt_price: 3.0, completion_price: 15.0, cached_read_price: 0.3, cached_write_price: 3.75 }));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    ...
    const get = await handler(authed("GET", "/api/admin/assistant/prices"));
  ...
  it("rejects invalid decimals", async () => {
    const res = await handler(authed("PUT", "/api/admin/assistant/prices", { model: "x", prompt_price: 0.1234567, completion_price: 0, cached_read_price: 0, cached_write_price: 0 }));
  ...
  it("non-admin → 403", async () => {
    const memberKey = "lxk_" + "m".repeat(43);
    const res = await handler(new Request("http://lexa.test/api/admin/assistant/prices", { method: "PUT", headers: { authorization: `Bearer ${memberKey}`, "content-type": "application/json" }, body: JSON.stringify({ model: "x", prompt_price: 0, completion_price: 0, cached_read_price: 0, cached_write_price: 0 }) }));
    expect(res.status).toBe(403);
  });
});

describe("GET /api/projects/:slug/assistant/usage", () => {
  it("returns scoped usage", async () => {
    const res = await handler(authed("GET", "/api/projects/proj1/assistant/usage"));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.summary.totalCalls).toBe(3);
    expect(body.byDay.length).toBe(3);
  ...
  it("unknown slug → 404", async () => {
    const res = await handler(authed("GET", "/api/projects/nonexistent/assistant/usage"));
    expect(res.status).toBe(404);
  });

  it("non-admin → 403", async () => {
    const memberKey = "lxk_" + "m".repeat(43);
    const res = await handler(new Request("http://lexa.test/api/projects/proj1/assistant/usage", { headers: { authorization: `Bearer ${memberKey}` } }));
    expect(res.status).toBe(403);
  });
});
