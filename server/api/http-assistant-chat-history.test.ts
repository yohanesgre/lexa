...
describe("GET /api/assistant/chats/:projectId", () => {
  ...
  it("member-bound key → 403 FORBIDDEN at the middleware", async () => {
    const res = await handler(authed("GET", "/api/assistant/chats/p1", undefined, MEMBER_KEY));
    expect(res.status).toBe(403);
    expect((await res.json()).error.code).toBe("FORBIDDEN");
  });

  it("no key → 401 UNAUTHORIZED", async () => {
    const res = await handler(pub("/api/assistant/chats/p1"));
    expect(res.status).toBe(401);
    expect((await res.json()).error.code).toBe("UNAUTHORIZED");
  });

  it("unknown project → 404 PROJECT_NOT_FOUND", async () => {
    const res = await handler(authed("GET", "/api/assistant/chats/ghost"));
    expect(res.status).toBe(404);
    expect((await res.json()).error.code).toBe("PROJECT_NOT_FOUND");
  });
});

describe("PATCH /api/assistant/chat/:chatId", () => {
  it("renames an owned thread (trimmed) and persists it", async () => {
    const res = await handler(authed("PATCH", "/api/assistant/chat/chat-b", { title: "  Renamed B  " }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ chatId: "chat-b", title: "Renamed B", pinned: false });
    const row = db.prepare(`SELECT title FROM assistant_threads WHERE document_id = 'chat-b'`).get() as { title: string | null };
    expect(row.title).toBe("Renamed B");
  });

  it("unknown chatId → 404 ASSISTANT_THREAD_NOT_FOUND", async () => {
    const res = await handler(authed("PATCH", "/api/assistant/chat/ghost", { title: "X" }));
    expect(res.status).toBe(404);
  ...
  it("another user's thread → 404 (owner mismatch indistinguishable)", async () => {
    ...
    const res = await handler(authed("PATCH", "/api/assistant/chat/chat-c", { title: "Hijack" }, ADMIN_KEY));
    expect(res.status).toBe(404);
    expect((await res.json()).error.code).toBe("ASSISTANT_THREAD_NOT_FOUND");
  });

  it("empty or over-long title → 422 INVALID_ARGS", async () => {
    const empty = await handler(authed("PATCH", "/api/assistant/chat/chat-b", { title: "   " }));
    expect(empty.status).toBe(422);
    expect((await empty.json()).error.code).toBe("INVALID_ARGS");
    const long = await handler(authed("PATCH", "/api/assistant/chat/chat-b", { title: "x".repeat(201) }));
    expect(long.status).toBe(422);
    expect((await long.json()).error.code).toBe("INVALID_ARGS");
  });
});

describe("DELETE /api/assistant/chat/:chatId", () => {
  it("another user's thread → 404", async () => {
    const res = await handler(authed("DELETE", "/api/assistant/chat/chat-c"));
    expect(res.status).toBe(404);
    expect((await res.json()).error.code).toBe("ASSISTANT_THREAD_NOT_FOUND");
    const still = db.prepare(`SELECT COUNT(*) AS c FROM assistant_threads WHERE document_id = 'chat-c'`).get() as { c: number };
    expect(still.c).toBe(1);
  });

  it("own thread → 204, gone afterwards", async () => {
    const res = await handler(authed("DELETE", "/api/assistant/chat/chat-del"));
    expect(res.status).toBe(204);
    expect(await res.text()).toBe("");
    const gone = await handler(authed("GET", "/api/assistant/chat/chat-del"));
    expect(gone.status).toBe(404);
    expect((await gone.json()).error.code).toBe("ASSISTANT_THREAD_NOT_FOUND");
  });

  it("delete while streaming → 409 ASSISTANT_TASK_ACTIVE; client abort releases the chat", async () => {
    ...
    for (let i = 0; i < 20 && !sawConflict; i++) {
      const del = await handler(authed("DELETE", "/api/assistant/chat/c-live"));
      if (del.status === 409) {
        sawConflict = true;
        expect((await del.json()).error.code).toBe("ASSISTANT_TASK_ACTIVE");
        break;
      }
    ...
    const released = await handler(authed("DELETE", "/api/assistant/chat/c-live"));
