        ...
        try {
          ...
          const consume = async (tools: ReadonlyArray<unknown> | undefined) => {
            ...
            for (;;) {
              ...
              try {
                result = await Promise.race([next, new Promise<never>((_, reject) => { stallTimer = setTimeout(() => { stalled = true; abort.abort(); reject(new AssistantGenerationFailed({ message: ASSISTANT_STALL_MESSAGE })); }, ASSISTANT_STALL_TIMEOUT_MS); })]);
              ...
              if (chunk.type === "TEXT_MESSAGE_CONTENT") {
                ...
                const name = rawName;
                const isWrite = isAssistantWriteTool(name);
                ...
                if (pending && pending.args !== "") {
                  ...
                  if (streamedArgs === undefined && pending.args.trim() !== "") {
                    ...
                    if (isWrite && streamedArgs === undefined) {
                      toolRounds += 1;
                      if (toolRounds > ctx.toolRoundCap) { abort.abort(); throw new AssistantToolBudgetExceeded({ rounds: ctx.toolRoundCap }); }
                      push({ type: "error", code: "ASSISTANT_TOOL_ARGS_INVALID", message: `Tool arguments invalid, skipping ${name || "tool"}` });
                      continue;
                    }
                  }
                }
              ...
              if (hallucinated || writeIntent) {
                ...
                try {
                  const line = JSON.stringify({
                    ...
                    message: `ASSISTANT_WRITE_HALLUCINATION_GUARD — ${reason}`,
                    meta: {
          ...
          if (!didFinish && !stalled) {
            const code = "ASSISTANT_GENERATION_FAILED";
            const message = ASSISTANT_STALL_MESSAGE;
          ...
          if (drained.length > 0) {
          ...
          } else if (e instanceof AssistantToolBudgetExceeded) {
            await persistTerminalTurn({ error: { code: "ASSISTANT_TOOL_BUDGET_EXCEEDED", message: `Assistant exceeded its tool budget (${e.rounds} rounds)` } }).catch(() => {});
            ...
            try {
              const line = JSON.stringify({ level, service: "assistant-build-stream", message: `[${(err as { _tag?: string })._tag ?? "AssistantGenerationFailed"}] ${message}`, meta: { threadId: ctx.threadId, keyId: ctx.keyId, idField: ctx.idField, code, errorTag: (err as { _tag?: string })._tag ?? null, status: (err as { status?: unknown }).status ?? null, providerMessage: (err as { providerMessage?: unknown }).providerMessage ?? null }, timestamp: new Date().toISOString() });
