...
describe("buildAdapter", () => {
  ...
  it("attaches x-opencode-session defaultHeaders on every adapter", () => {
    for (const cfg of [openaiConfig(), anthropicConfig(), responsesConfig()]) {
      ...
      expect(headers?.[OPENCODE_SESSION_HEADER]).toBe("lexa-assistant-thread-abc");
    }
  });
...
describe("opencode session id", () => {
  it("prefixes bare conversation ids with lexa-assistant-", () => {
    expect(opencodeSessionIdFor("thread-abc")).toBe("lexa-assistant-thread-abc");
    expect(opencodeSessionIdFor("lexa-assistant-thread-abc")).toBe("lexa-assistant-thread-abc");
  });

  it("resolveOpencodeSessionId prefers the conversation id, falls back per model", () => {
    expect(resolveOpencodeSessionId("chat-1", { model: "m" })).toBe("lexa-assistant-chat-1");
    expect(resolveOpencodeSessionId(undefined, { model: "m" })).toBe("lexa-assistant-m");
    expect(resolveOpencodeSessionId(undefined, { providerId: "p1", model: "m" })).toBe("lexa-assistant-p1");
    expect(resolveOpencodeSessionId("  ", { model: "m" })).toBe("lexa-assistant-m");
  });
});
...
describe("listModels", () => {
  ...
  it("sends x-opencode-session on the openai wire (fallback per model)", async () => {
    ...
    expect(log.headers?.[OPENCODE_SESSION_HEADER]).toBe("lexa-assistant-gpt-4o");
  });
...
describe("translateRunError", () => {
  ...
  it("falls through to AssistantGenerationFailed with bounded message", () => {
  ...
  it("stringifies non-Error throwables", () => {
    expect(translateRunError("boom")._tag).toBe("AssistantGenerationFailed");
  });
...
describe("translateRunError mapping errors", () => {
  it("JSON parse error -> AssistantGenerationFailed with mapping message and preserves retryAfter", () => {
    ...
    expect(translated._tag).toBe("AssistantGenerationFailed");
  ...
  it("tool_call XML leak -> AssistantGenerationFailed mapping", () => {
  ...
  it("5xx string status '503' -> AssistantGenerationFailed with status", () => {
...
describe("assistantLog levels", () => {
  it("FATAL emitted for mapping error with errorTag", async () => {
    ...
    try {
      ...
      expect(t._tag).toBe("AssistantGenerationFailed");
    } finally {
      process.stderr.write = origStderr;
      process.stdout.write = origStdout;
    }
    const fatal = writes.find((s) => s.includes('"level":"FATAL"') && s.includes("AssistantGenerationFailed"));
