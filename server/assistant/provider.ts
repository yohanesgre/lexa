...
export type AssistantLogLevel = "DEBUG" | "INFO" | "WARN" | "ERROR" | "FATAL";
...
export function assistantLog(level: AssistantLogLevel, message: string, meta?: Record<string, unknown>): void {
...
export function normalizeBaseUrl(raw: string, kind: ProviderKind | string): string {
  ...
  assistantLog("DEBUG", "assistant-provider normalizeBaseUrl", { raw, kind: coerced, normalized });
  return normalized;
}
...
export function translateRunError(e: unknown, config?: ProviderConfig): ProviderAuthFailed | ProviderUnreachable | AssistantGenerationFailed {
  const parts: string[] = [];
  collectErrorText(e, parts);
  ...
  if (isMappingError) {
    ...
    assistantLog("FATAL", `[AssistantGenerationFailed] ${final}`, {
      errorTag: "AssistantGenerationFailed",
    ...
    return new AssistantGenerationFailed({
      message: final,
      status: statusCode ?? null,
      providerMessage: extractProviderMessage(e) ?? null,
      raw: raw.slice(0, 800),
  ...
  if (
    ...
    assistantLog("FATAL", `[ProviderAuthFailed] ${raw.slice(0, 500)}`, {
  ...
  if (
    ...
    assistantLog("WARN", `[ProviderUnreachable] ${raw.slice(0, 500) || "rate limited"}`, {
  ...
  if (
    ...
    assistantLog("WARN", `[ProviderUnreachable] ${raw.slice(0, 500)}`, {
      errorTag: "ProviderUnreachable",
      status: statusCode ?? null,
      providerMessage: providerMessagePreview ?? null,
      raw: raw.slice(0, 800),
  ...
  const rawEventJson = extractRawEventJson(e);
  {
    ...
    let level: AssistantLogLevel = "ERROR";
    if (isNonRetriableStatus) level = "FATAL";
    else if (isTransient) level = "WARN";
    assistantLog(level, `[AssistantGenerationFailed] ${finalMessage}`, {
      errorTag: "AssistantGenerationFailed",
      status: statusCode ?? null,
