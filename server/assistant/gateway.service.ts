...
export function gatewayLog(level: GatewayLogLevel, message: string, meta: Record<string, unknown>): void {
  ...
  try {
    const line = JSON.stringify({ level, service: "assistant-gateway", message, meta, timestamp: new Date().toISOString() });
...
async function sleepBackoff(e: unknown, attempt: number, metaExtra?: Record<string, unknown>): Promise<void> {
  ...
  gatewayLog("WARN", `assistant backoff ${ms}ms before retry`, { attempt: attempt + 1, backoffMs: ms, status, providerMessage: extractProviderMessage(e) ?? null, errorTag, retryAfter, base: status === 429 && retryAfter !== null ? Math.min(retryAfter, 8000) : status === 429 ? 400 : 800, ...(metaExtra ?? {}) });
  gatewayLog("DEBUG", `assistant backoff debug`, { attempt: attempt + 1, backoffMs: ms, status, errorTag, retryAfter, abortSignalled: false });
...
function isRetriable(e: unknown): boolean {
  ...
  if (tag === "AssistantGenerationFailed") {
...
export class AssistantGateway extends Effect.Service<AssistantGateway>()("Lexa/AssistantGateway", {
  ...
  effect: Effect.gen(function* () {
    ...
    const healthOption = yield* Effect.serviceOption(AssistantHealthService);
            ...
            if (primaryCfg) {
                ...
                if (fid.includes(":")) {
                  ...
                  if (!m) {
                    ...
                    if (byMid) {
                      ...
                      if (!providerFilter || byMidPid === providerFilter) {
                        gatewayLog("WARN", `assistant fallback ${fid} not found via byProviderModel, using byModelId ${midPart}`, { fid, fallbackModel: midPart, providerId: byMidPid ?? null });
                        m = byMid;
                      } else {
                        gatewayLog("WARN", `assistant fallback ${fid} not found`, { fid, reason: "provider mismatch for byModelId fallback" });
                      }
                    } else {
                      ...
                      if (anyForProvider) {
                        ...
                        gatewayLog("WARN", `assistant fallback ${fid} not found, using any enabled model for provider ${fPid}`, { fid, fallbackModel: anyMid, providerId: fPid });
                        m = anyForProvider;
                      } else {
                        gatewayLog("WARN", `assistant fallback ${fid} not found`, { fid, reason: "no enabled model for provider" });
                      }
                    }
                  }
                  ...
                  if (!m) {
                    ...
                    if (byMid) {
                      const byMidPid = (byMid as { provider_id?: string; providerId?: string }).provider_id ?? (byMid as { providerId?: string }).providerId;
                      if (!providerFilter || byMidPid === providerFilter) {
                        gatewayLog("WARN", `assistant fallback ${fid} not found in scopedModels, using byModelId`, { fid, providerId: byMidPid ?? null });
                        m = byMid;
                      }
                    }
                    if (!m) gatewayLog("WARN", `assistant fallback ${fid} not found`, { fid });
                  }
                } else {
                  m = byModelId.get(fid);
                  if (!m) gatewayLog("WARN", `assistant fallback ${fid} not found via byModelId`, { fid });
                }
                if (!m) continue;
              ...
              if (configs.length === 1) {
                for (const m of enabledModels) {
                  ...
                  if (providerFilter && cfg.providerId !== providerFilter) continue;
                  if (configs.some((c) => c.providerId === cfg.providerId && c.model === cfg.model)) continue;
                  gatewayLog("WARN", `assistant fallback supplemented to ensure >1 config`, { addedModel: cfg.model, providerId: cfg.providerId });
                  configs.push(cfg);
                }
              }
              if (configs.length > 0) {
      ...
      return {
        [Symbol.asyncIterator]: async function* () {
          ...
          for (let i = 0; i < attemptConfigs.length; i++) {
            gatewayLog("DEBUG", "assistant stream attempt abort check", { projectId: input.projectId, attempt: i + 1, total: attemptConfigs.length, aborted: !!abortSignal?.aborted, abortReason: (() => { try { return abortSignal?.reason ? String((abortSignal as unknown as { reason: unknown }).reason).slice(0, 200) : null; } catch { return null; } })() });
            if (abortSignal?.aborted) {
              gatewayLog("INFO", `assistant stream aborted before attempt ${i + 1}`, { projectId: input.projectId, attempt: i + 1, total: attemptConfigs.length });
              break;
            }
            const cfg = attemptConfigs[i]!;
            gatewayLog("DEBUG", "assistant stream attempt config", { projectId: input.projectId, attempt: i + 1, total: attemptConfigs.length, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), apiKeyMask: maskApiKeyShort(cfg.apiKey) });
            if (cfg.providerId) {
              try {
                const allowed = await Effect.runPromise(health.isAllowed(cfg.providerId).pipe(Effect.catchAll(() => Effect.succeed(true))));
                if (!allowed) {
                  gatewayLog("DEBUG", "assistant stream health blocked", { projectId: input.projectId, providerId: cfg.providerId, attempt: i + 1 });
                  continue;
                }
              } catch {}
            }
            ...
            try {
              ...
              if (isSingleConfig && !retriedSingleNetwork && isSingleRetryCandidate(err) && i === 0) {
                ...
                gatewayLog("WARN", `assistant stream retrying next model after baseUrl failure`, { projectId: input.projectId, attempt: i + 1, nextModel: attemptConfigs[i + 1]?.model ?? null, singleRetry: true, status: diag.status, errorTag: (err as { _tag?: string })._tag ?? null, retryAfter: diag.retryAfter, backoffMs: backoffMsFor(err, 0) });
              ...
              gatewayLog("WARN", `assistant stream retrying next model after baseUrl failure`, { projectId: input.projectId, attempt: i + 1, nextModel: attemptConfigs[i + 1]?.model ?? null, singleRetry: isSingleRetry && i === 0, status: diag.status, errorTag: (err as { _tag?: string })._tag ?? null, retryAfter: diag.retryAfter, backoffMs: backoffMsFor(err, i) });
              continue;
            }
            try {
              void buildAdapter(input.sessionId !== undefined ? { ...cfg, sessionId: input.sessionId } : cfg);
              gatewayLog("DEBUG", "assistant stream dispatch config snapshot", { projectId: input.projectId, attempt: i + 1, total: attemptConfigs.length, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), apiKeyMask: maskApiKeyShort(cfg.apiKey) });
              gatewayLog("INFO", `assistant stream dispatch attempt ${i + 1}/${attemptConfigs.length}${singleRetryLabel}`, { attempt: i + 1, total: attemptConfigs.length, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), singleRetry: isSingleRetry && i === 1 });
              ...
              for await (const chunk of iterable) {
                if ((chunk as { type?: string }).type === "RUN_ERROR") {
                  ...
                  gatewayLog("ERROR", `assistant RUN_ERROR chunk received before translate`, { projectId: input.projectId, providerId: cfg.providerId ?? null, model: cfg.model, code: c.code ?? null, message: String(c.message ?? "").slice(0, 500), rawEvent: c.rawEvent ?? null });
              ...
              const diag = diagFromError(e);
              const err = tagged === "ProviderAuthFailed" || tagged === "ProviderUnreachable" || tagged === "AssistantGenerationFailed" ? (e as ReturnType<typeof translateRunError>) : translateRunError(e, cfg);
              lastError = err;
              attempts.push({ model: cfg.model, providerId: cfg.providerId ?? null, status: diag.status, providerMessage: diag.providerMessage, raw: diag.raw.slice(0, 500), errorTag: (err as { _tag?: string })._tag ?? null, retryAfter: diag.retryAfter });
          ...
          throw new AssistantGenerationFailed({ message: "all models failed", attempts } as never);
        },
    ...
    const testConnection = (
      ...
      Effect.gen(function* () {
        if (override?.kind && override?.baseUrl && override?.model) {
          ...
          yield* Effect.sync(() => gatewayLog("DEBUG", "assistant test direct config", { projectId, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), apiKeyMask: maskApiKeyShort(cfg.apiKey) }));
          yield* Effect.sync(() => gatewayLog("INFO", `assistant test direct start`, { projectId, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), apiKeyMask: maskApiKeyShort(cfg.apiKey) }));
          const res = yield* Effect.tryPromise({
            try: () => providerTestConnection(cfg, opts),
            catch: (e) => {
              const diag = diagFromError(e);
              gatewayLog("ERROR", `assistant test direct failed — ${(e as { message?: string })?.message ?? String(e)}`, { projectId, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), apiKeyMask: maskApiKeyShort(cfg.apiKey), status: diag.status, providerMessage: diag.providerMessage, raw: diag.raw.slice(0, 800), rawEvent: diag.rawEvent, stack: diag.stack, causeChain: diag.causeChain, errorTag: (e as { _tag?: string })?._tag ?? null, retryAfter: diag.retryAfter });
              return translateRunError(e, cfg) as never;
            },
          }).pipe(Effect.tap((r) => Effect.sync(() => gatewayLog("INFO", `assistant test direct succeeded`, { projectId, model: cfg.model, latencyMs: r.latencyMs }))));
          return res;
        }
            ...
            if (fid.includes(":")) {
              ...
              if (!m) {
                ...
                if (byMid) {
                  const byMidPid = (byMid as { provider_id?: string; providerId?: string }).provider_id ?? (byMid as { providerId?: string }).providerId;
                  if (byMidPid === pid) {
                    gatewayLog("WARN", `assistant fallback ${fid} not found via byProviderModel, using byModelId ${midPart}`, { fid, fallbackModel: midPart, providerId: pid });
                    m = byMid;
                  } else {
                    gatewayLog("WARN", `assistant fallback ${fid} not found`, { fid, reason: "provider mismatch for byModelId fallback" });
                  }
                  ...
                  if (anyForProvider) { m = anyForProvider; gatewayLog("WARN", `assistant fallback ${fid} not found, using enabled model for provider`, { fid, providerId: pid }); }
                  else gatewayLog("WARN", `assistant fallback ${fid} not found`, { fid });
                }
              }
            } else {
              m = byModelId.get(fid);
        ...
        for (let i = 0; i < attemptConfigs.length; i++) {
          yield* Effect.sync(() => gatewayLog("DEBUG", "assistant test attempt abort check", { projectId, attempt: i + 1, total: attemptConfigs.length, aborted: !!opts?.signal?.aborted }));
          if (opts?.signal?.aborted) {
            yield* Effect.sync(() => gatewayLog("INFO", `assistant test aborted before attempt ${i + 1}`, { projectId, attempt: i + 1, total: attemptConfigs.length }));
            break;
          }
          const cfg = attemptConfigs[i]!;
          yield* Effect.sync(() => gatewayLog("DEBUG", "assistant test attempt config", { projectId, attempt: i + 1, total: attemptConfigs.length, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), apiKeyMask: maskApiKeyShort(cfg.apiKey) }));
          const singleRetryLabel = isSingleRetry && i === 1 ? " (single-config retry)" : "";
          const attemptStart = Date.now();
          yield* Effect.sync(() => gatewayLog("INFO", `assistant test attempt ${i + 1}/${attemptConfigs.length}${singleRetryLabel} start`, { projectId, attempt: i + 1, total: attemptConfigs.length, providerId: cfg.providerId ?? null, model: cfg.model, kind: normalizeProviderKind(cfg.kind), baseUrl: safeBaseUrl(cfg.baseUrl, cfg.kind), apiKeyMask: maskApiKeyShort(cfg.apiKey), singleRetry: isSingleRetry && i === 1 }));
          try {
            const res = yield* Effect.tryPromise({
              try: () => providerTestConnection(cfg, opts),
              catch: (e) => translateRunError(e, cfg) as never,
            });
            yield* Effect.sync(() => gatewayLog("INFO", `assistant test attempt ${i + 1}/${attemptConfigs.length}${singleRetryLabel} succeeded`, { attempt: i + 1, total: attemptConfigs.length, providerId: cfg.providerId ?? null, model: cfg.model, latencyMs: res.latencyMs, singleRetry: isSingleRetry && i === 1 }));
            success = res;
            break;
          } catch (e) {
            const diag = diagFromError(e);
            ...
            if (i === attemptConfigs.length - 1) {
              if (attempts.length > 1 || attemptConfigs.length > 1) {
                ...
                const combined = `all models failed — ${detail}`.slice(0, 500);
                yield* Effect.sync(() => gatewayLog("FATAL", `assistant test all ${attemptConfigs.length} models failed — aggregate`, { projectId, total: attemptConfigs.length, attempts, combined, singleRetry: isSingleRetry, status: diag.status, errorTag: tag ?? null, retryAfter: diag.retryAfter }));
                throw new AssistantGenerationFailed({ message: combined, status: diag.status, providerMessage: diag.providerMessage, raw: diag.raw.slice(0, 500), attempts, errorTag: tag ?? null } as never);
              }
              throw err as never;
            }
            if (isRetriable(err)) yield* Effect.promise(() => sleepBackoff(err, i, { projectId, model: cfg.model, providerId: cfg.providerId ?? null }));
            yield* Effect.sync(() => gatewayLog("WARN", `assistant test retrying next model${isSingleRetry && i === 0 ? " (single-config retry same provider)" : ""}`, { projectId, attempt: i + 1, nextModel: attemptConfigs[i + 1]?.model ?? null, nextProviderId: attemptConfigs[i + 1]?.providerId ?? null, failuresSoFar: attempts.length, singleRetry: isSingleRetry && i === 0, status: diag.status, errorTag: tag ?? null, retryAfter: diag.retryAfter, backoffMs: backoffMsFor(err, i) }));
          }
        }
