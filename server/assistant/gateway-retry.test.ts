...
function stubProviders() {
  return Layer.succeed(AssistantProvidersRepo, { list: () => Effect.succeed([]), listAll: () => Effect.succeed([]), getById: () => Effect.fail(new (class E { _tag = "RowNotFound" as const; table = "assistant_providers" })()) } as unknown as never);
}
function stubModels() {
  return Layer.succeed(AssistantModelsRepo, { listAll: () => Effect.succeed([]), listByProvider: () => Effect.succeed([]) } as unknown as never);
}
function stubSettings() {
  return Layer.succeed(AssistantSettingsRepo, { getByProject: () => Effect.fail(new (class E { _tag = "RowNotFound" as const; table = "assistant_settings" })()), maskedView: () => Effect.fail(new (class E { _tag = "RowNotFound" as const; table = "assistant_settings" })()), upsert: () => Effect.succeed(null as never) } as unknown as never);
}
...
describe("AssistantGateway retry/backoff", () => {
  ...
  it("400 non-retriable does not retry fallback", async () => {
    ...
    spy.mockImplementation((() =>
      (async function* () {
        throw new AssistantGenerationFailed({ message: "400 bad request", status: 400 } as never);
      })()) as never);
    const gatewayLayer = AssistantGateway.Default.pipe(Layer.provide(stubProviders()), Layer.provide(stubModels()), Layer.provide(stubCallLog()), Layer.provide(stubSettings()), Layer.provide(memDbLayer()));
    ...
    spy.mockImplementation((() => {
      ...
      if (callCount === 1) {
        return (async function* () {
          throw Object.assign(new AssistantGenerationFailed({ message: "429 rate limit", status: 429 } as never), { status: 429 });
        })();
      }
      return (async function* () {
  ...
  it("500 retriable does retry", async () => {
    ...
    spy.mockImplementationOnce((() =>
      (async function* () {
        throw new AssistantGenerationFailed({ message: "500 Internal", status: 500 } as never);
      })()) as never);
    spy.mockImplementationOnce((() =>
      (async function* () {
  ...
  it("mapping error not retriable, preserves message", async () => {
    ...
    spy.mockImplementation((() =>
      (async function* () {
        throw new AssistantGenerationFailed({ message: "upstream response mapping failed: Unexpected token" } as never);
      })()) as never);
    const gatewayLayer = AssistantGateway.Default.pipe(Layer.provide(stubProviders()), Layer.provide(stubModels()), Layer.provide(stubCallLog()), Layer.provide(stubSettings()), Layer.provide(memDbLayer()));
    const program = Effect.gen(function* () {
      const gw = yield* AssistantGateway;
    ...
    expect(err._tag).toBe("AssistantGenerationFailed");
  ...
  it("single-config 429 retries same model with backoff", async () => {
    ...
    spy.mockImplementation((() => {
      ...
      if (count === 1) {
        return (async function* () {
          throw new AssistantGenerationFailed({ message: "429 rate limit", status: 429 } as never);
        })();
      }
      return (async function* () {
  ...
  it("FATAL emitted on all-models-failed terminal", async () => {
    const spy = vi.spyOn(provider, "streamChat");
    spy.mockImplementation((() => (async function* () { throw new AssistantGenerationFailed({ message: "500 Internal", status: 500 } as never); })()) as never);
    const writes: string[] = [];
