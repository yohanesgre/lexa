...
export class AssistantChatService extends Effect.Service<AssistantChatService>()("Lexa/AssistantChatService", {
  ...
  effect: Effect.gen(function* () {
    ...
    return {
      ...
      runChatStream: (chatId: string, userId: string, req: AssistantChatStreamRequest) => Effect.gen(function* () {
        ...
        yield* queryFirst<{ one: number }>(db, `SELECT 1 AS one FROM assistant_threads WHERE document_type = 'chat' AND document_id = ? LIMIT 1`, chatId).pipe(
        ...
        const baseModelOptions = modelOptionsForEffort(
          resolveReasoningEffort(
            (settingsRow as unknown as { reasoning_effort: import("../../shared/assistant").AssistantReasoningEffort | null }).reasoning_effort,
            req.reasoningEffort
          )
        );
