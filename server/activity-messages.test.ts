...
describe("activity messages", () => {
  it("formats per type", () => {
    ...
    expect(m.runtimeCompleted("opencode")).toBe("AI: opencode completed — result ready");
  ...
  it("formatActivityMessage dispatches per type", () => {
    ...
    expect(m.formatActivityMessage("runtime_completed", { agent: "opencode" })).toBe("AI: opencode completed — result ready");
    expect(m.formatActivityMessage("runtime_failed", {})).toBe("AI run failed");
  });
  ...
  it("formatActivityMessage covers every ActivityType with valid payloads", () => {
    const cases: [ActivityType, m.ActivityMessagePayload, string][] = [
      ...
      ["runtime_completed", { agent: "opencode" }, "AI: opencode completed — result ready"],
      ["runtime_failed", {}, "AI run failed"],
      ["runtime_cancelled", {}, "AI run cancelled"],
