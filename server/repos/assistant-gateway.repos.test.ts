...
describe("assistant gateway phase 1", () => {
  ...
  it("assistant_settings upsert/maskedView without legacy cols", async () => {
    const { dbPath, dir } = tempDbPath();
    try {
      runMigrations(dbPath);
      ...
      seed.close();
      const db = new Database(dbPath);
      const layer = Layer.mergeAll(AssistantSettingsRepo.Default).pipe(Layer.provide(Layer.mergeAll(Layer.succeed(Sqlite, db), DbBunLive(db))));
  ...
  it("models + call_logs + prices round-trip and constraints", async () => {
    ...
    try {
      ...
      seed.close();
      const db = new Database(dbPath);
      const layer = Layer.mergeAll(AssistantProvidersRepo.Default, AssistantModelsRepo.Default, AssistantCallLogsRepo.Default, AssistantModelPricesRepo.Default).pipe(Layer.provide(Layer.mergeAll(Layer.succeed(Sqlite, db), DbBunLive(db))));
      const prog = Effect.gen(function* () {
        const provRepo = yield* AssistantProvidersRepo;
        ...
        const price = yield* priceRepo.upsert({ model: "gpt-4o", promptPrice: 3.0, completionPrice: 15.0, cachedReadPrice: 0.3, cachedWritePrice: 3.75 });
        expect(price.promptPrice).toBe(3.0);
        const fetched = yield* priceRepo.getByModel("gpt-4o");
        expect(fetched.completionPrice).toBe(15.0);
        expect(fetched.cachedReadPrice).toBe(0.3);
        expect(fetched.cachedWritePrice).toBe(3.75);
        const price2 = yield* priceRepo.upsert({ model: "gpt-4o", promptPrice: 3.5, completionPrice: 17.5, cachedReadPrice: 0.35, cachedWritePrice: 4.0 });
        expect(price2.promptPrice).toBe(3.5);
      });
      await Effect.runPromise(prog.pipe(Effect.provide(layer)));
      db.close();
    } finally { rmSync(dir, { recursive: true, force: true }); }
