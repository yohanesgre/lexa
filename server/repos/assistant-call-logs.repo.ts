...
import type { AssistantCallLogRow, AssistantCallLogInput } from "../../shared/assistant";

export interface AssistantCallLogDbRow {
  id: string;
...
function toDomain(row: AssistantCallLogDbRow): AssistantCallLogRow {
  return {
    id: row.id,
    ...
    return {
      insert: (input: { id: string } & AssistantCallLogInput): Effect.Effect<AssistantCallLogRow, ConstraintViolation | DbError | RowNotFound> =>
        run(
          ...
          Effect.flatMap(() => queryFirst<AssistantCallLogDbRow>(db, `SELECT * FROM assistant_call_logs WHERE id = ?`, input.id)),
          Effect.map(toDomain)
        ),

      getById: (id: string): Effect.Effect<AssistantCallLogRow, RowNotFound | DbError> =>
        Effect.map(queryFirst<AssistantCallLogDbRow>(db, `SELECT * FROM assistant_call_logs WHERE id = ?`, id), toDomain),

      listByProject: (projectId: string, limit = 100): Effect.Effect<AssistantCallLogRow[], DbError> =>
        Effect.map(
          queryAll<AssistantCallLogDbRow>(db, `SELECT * FROM assistant_call_logs WHERE project_id = ? ORDER BY created_at DESC LIMIT ?`, projectId, limit),
          (rows) => rows.map(toDomain)
        ),

      listByProvider: (providerId: string, limit = 100): Effect.Effect<AssistantCallLogRow[], DbError> =>
        Effect.map(
          queryAll<AssistantCallLogDbRow>(db, `SELECT * FROM assistant_call_logs WHERE provider_id = ? ORDER BY created_at DESC LIMIT ?`, providerId, limit),
          (rows) => rows.map(toDomain)
        ),

      listByModel: (model: string, limit = 100): Effect.Effect<AssistantCallLogRow[], DbError> =>
        Effect.map(
          queryAll<AssistantCallLogDbRow>(db, `SELECT * FROM assistant_call_logs WHERE model = ? ORDER BY created_at DESC LIMIT ?`, model, limit),
          (rows) => rows.map(toDomain)
        ),

      listRecent: (limit = 100): Effect.Effect<AssistantCallLogRow[], DbError> =>
        Effect.map(
          queryAll<AssistantCallLogDbRow>(db, `SELECT * FROM assistant_call_logs ORDER BY created_at DESC LIMIT ?`, limit),
          (rows) => rows.map(toDomain)
        ),
        ...
        Effect.gen(function* () {
          ...
          const row = yield* queryFirst<{
            ...
            `WITH filtered AS (
                ...
                FROM assistant_call_logs ${where}
             ),
             lat AS (
                SELECT latency_ms,
