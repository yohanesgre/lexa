...
export class SwimlaneRepo extends Effect.Service<SwimlaneRepo>()("Lexa/SwimlaneRepo", {
  effect: Effect.gen(function* () {
    ...
    return {
      create: (input: { id: string; projectId: string; name: string; position: number }): Effect.Effect<Swimlane, ConstraintViolation | DbError | RowNotFound> =>
        Effect.gen(function* () {
          yield* run(db,
            `INSERT INTO swimlanes (id, project_id, name, position) VALUES (?, ?, ?, ?)`,
            input.id, input.projectId, input.name, input.position
          );
      ...
      update: (id: string, input: { name?: string; position?: number }): Effect.Effect<Swimlane, RowNotFound | DbError | ConstraintViolation> => {
        const sets: string[] = [];
