...
export interface AssistantPendingWriteRow {
  id: string;
  project_id: string;
...
export class AssistantPendingWritesRepo extends Effect.Service<AssistantPendingWritesRepo>()("Lexa/AssistantPendingWritesRepo", {
  effect: Effect.gen(function* () {
    ...
    const insert = (
      row: Omit<AssistantPendingWriteRow, "status" | "execution_error" | "created_at" | "decided_at">
    ...
    const sweepExpiredInternal = (): Effect.Effect<number, DbError | ConstraintViolation> =>
      run(db, `UPDATE assistant_pending_writes SET status = 'expired' WHERE status = 'pending' AND expires_at <= datetime('now')`).pipe(Effect.catchAll(() => Effect.succeed(0 as number)));

    const getById = (id: string): Effect.Effect<AssistantPendingWriteRow | null, DbError> =>
      sweepExpiredInternal().pipe(
        Effect.flatMap(() => queryFirst<AssistantPendingWriteRow>(db, `SELECT * FROM assistant_pending_writes WHERE id = ?`, id).pipe(Effect.catchTag("RowNotFound", () => Effect.succeed(null)))),
        Effect.catchAll(() => queryFirst<AssistantPendingWriteRow>(db, `SELECT * FROM assistant_pending_writes WHERE id = ?`, id).pipe(Effect.catchTag("RowNotFound", () => Effect.succeed(null))))
      );
    ...
    const decide = (id: string, status: "approved" | "rejected"): Effect.Effect<AssistantPendingWriteRow | null, DbError | ConstraintViolation> =>
      queryFirst<AssistantPendingWriteRow>(
        db,
        `UPDATE assistant_pending_writes SET status = ?, decided_at = datetime('now')
    ...
    const expireIfDue = (id: string): Effect.Effect<AssistantPendingWriteRow | null, DbError> =>
      queryFirst<AssistantPendingWriteRow>(
        db,
        `UPDATE assistant_pending_writes SET status = 'expired'
    ...
    const sweepExpired = (): Effect.Effect<number, DbError | ConstraintViolation> =>
      run(db, `UPDATE assistant_pending_writes SET status = 'expired'
               WHERE status = 'pending' AND expires_at <= datetime('now')`);

    const listByBatch = (batchId: string): Effect.Effect<AssistantPendingWriteRow[], DbError> =>
      sweepExpiredInternal().pipe(
        Effect.flatMap(() =>
          queryAll<AssistantPendingWriteRow>(db, `SELECT * FROM assistant_pending_writes WHERE batch_id = ? ORDER BY seq ASC`, batchId)
        ),
        Effect.catchAll(() => queryAll<AssistantPendingWriteRow>(db, `SELECT * FROM assistant_pending_writes WHERE batch_id = ? ORDER BY seq ASC`, batchId))
      );

    const countByBatchRemaining = (batchId: string): Effect.Effect<number, DbError> =>
      sweepExpiredInternal().pipe(
        Effect.flatMap(() =>
          queryFirst<{ n: number }>(db, `SELECT COUNT(*) AS n FROM assistant_pending_writes WHERE batch_id = ? AND status = 'pending'`, batchId).pipe(
            Effect.map((r) => r.n),
    ...
    const markExecutionError = (id: string, error: string): Effect.Effect<void, DbError | ConstraintViolation> =>
      Effect.asVoid(run(db, `UPDATE assistant_pending_writes SET execution_error = ? WHERE id = ?`, error, id));
