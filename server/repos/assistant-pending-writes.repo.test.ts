...
import { AssistantPendingWritesRepo, type AssistantPendingWriteRow } from "./assistant-pending-writes.repo";
...
beforeAll(() => {
  dir = mkdtempSync(join(tmpdir(), "lexa-assistant-pending-writes-repo-"));
...
// assistant_pending_writes FK targets: projects, users, and the composite
// assistant_threads(document_type, document_id) key.
function seed(db: Database) {
  db.exec(`
    INSERT INTO projects (id, name, slug) VALUES ('p1', 'P', 'p1');
    INSERT INTO users (id, email, name, role) VALUES ('u1', 'u1@x', 'U1', 'member');
    INSERT INTO assistant_threads (document_type, document_id, project_id)
...
function seedDecided(db: Database, id: string, status: string) {
  db.prepare(`UPDATE assistant_pending_writes SET status = ? WHERE id = ?`).run(status, id);
}

function makeRepo(db: Database) {
  const layer = AssistantPendingWritesRepo.Default.pipe(Layer.provide(Layer.mergeAll(Layer.succeed(Sqlite, db), DbBunLive(db))));
  const ctx = Effect.runSync(Effect.scoped(Layer.build(layer)));
  return Context.get(ctx, AssistantPendingWritesRepo);
}

function row(overrides: Partial<AssistantPendingWriteRow>): AssistantPendingWriteRow {
  return {
...
describe("AssistantPendingWritesRepo insert/getById", () => {
  ...
  it("FK enforcement: insert without the assistant_threads composite key fails", async () => {
    seed(db);
    const repo = makeRepo(db);
    await Effect.runPromise(
      Effect.gen(function* () {
...
describe("AssistantPendingWritesRepo decide guard paths", () => {
...
describe("AssistantPendingWritesRepo expireIfDue / sweepExpired", () => {
...
describe("AssistantPendingWritesRepo listByBatch / countByBatchRemaining", () => {
