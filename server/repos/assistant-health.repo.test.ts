...
import { AssistantHealthRepo } from "./assistant-health.repo";
...
describe("assistant-health.repo", () => {
  it("migration creates assistant_provider_health with correct schema and FK cascade", async () => {
    const { dbPath, dir } = tempDbPath();
    try {
      runMigrations(dbPath);
      const db = new Database(dbPath);
      const row = db.prepare("SELECT sql FROM sqlite_master WHERE name='assistant_provider_health'").get() as { sql: string };
      expect(row.sql).toContain("provider_id TEXT PRIMARY KEY");
      expect(row.sql).toMatch(/REFERENCES\s+"?assistant_providers"?\(id\) ON DELETE CASCADE/);
  ...
  it("upsert + get + delete round-trip", async () => {
    ...
    try {
      ...
      const layer = AssistantHealthRepo.Default.pipe(Layer.provide(Layer.mergeAll(Layer.succeed(Sqlite, db), DbBunLive(db))));
  ...
  it("cascade delete removes health row when provider deleted", async () => {
    ...
    try {
      ...
      const prog = Effect.gen(function* () {
        ...
        db.prepare("DELETE FROM assistant_providers WHERE id='pr1'").run();
