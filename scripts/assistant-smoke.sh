...
resp=$(api_body "${auth[@]}" -w '\n%{http_code}' -X POST "$BASE_URL/api/assistant/tasks" \
...
resp=$(api_body "${auth[@]}" -w '\n%{http_code}' -X POST "$BASE_URL/api/assistant/settings/$PROJECT_ID/test" \
...
resp=$(api_body "${auth[@]}" -X POST "$BASE_URL/api/assistant/settings/$PROJECT_ID/models" \
...
resp=$(api_body "${auth[@]}" -X PUT "$BASE_URL/api/assistant/settings/$PROJECT_ID" \
  -d "{\"kind\":\"openai_compatible\",\"baseUrl\":\"http://localhost:$MOCK_PORT\",\"model\":\"mock-mini\",\"apiKey\":\"$MOCK_KEY\"}")
echo "$resp"
...
echo
echo "== 6. memory CRUD =="
mem=$(api_body "${auth[@]}" -X POST "$BASE_URL/api/assistant/memory/$PROJECT_ID" -d '{"content":"Prefers terse answers"}')
MEM_ID=$(echo "$mem" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')
list=$(api_body "${auth[@]}" "$BASE_URL/api/assistant/memory/$PROJECT_ID")
echo "$list" | grep -q "Prefers terse answers" || fail "memory list missing entry"
code=$(api "${auth[@]}" -X DELETE "$BASE_URL/api/assistant/memory/$PROJECT_ID/$MEM_ID")
...
created=$(api_body "${auth[@]}" -X POST "$BASE_URL/api/assistant/tasks" \
...
echo "-- frames:"
curl -sS -N --max-time 30 "${auth[@]}" -X POST "$BASE_URL/api/assistant/tasks/$TASK_ID/stream" | head -20
status=$(api_body "${auth[@]}" "$BASE_URL/api/runtimes/tasks/$TASK_ID")
...
CHAT_ID="smoke-chat-$$"
echo "-- frames:"
curl -sS -N --max-time 30 "${auth[@]}" -X POST "$BASE_URL/api/assistant/chat/stream" \
  -d "{\"projectId\":\"$PROJECT_ID\",\"chatId\":\"$CHAT_ID\",\"message\":\"hello\"}" | head -10
tr=$(api_body "${auth[@]}" "$BASE_URL/api/assistant/chat/$CHAT_ID")
echo "-- transcript:"
echo "$tr" | python3 -c 'import json,sys; d=json.load(sys.stdin); print("messages:", len(d["messages"]), "| summary:", d["summary"])'
code=$(api "${auth[@]}" -X DELETE "$BASE_URL/api/assistant/chat/$CHAT_ID")
[ "$code" = "204" ] || fail "chat reset got $code"
code=$(api "${auth[@]}" "$BASE_URL/api/assistant/chat/$CHAT_ID")
