...
export const Route = createFileRoute("/runtimes/usage")({
  ...
  ssr:false,
  component: RuntimeUsageRoute,
});

function RuntimeUsageRoute() {
  const { canViewUsage, isLoading } = useRuntimeRole();
      ...
      <section className="card-panel mt-4">
        ...
        <div className="flex items-center gap-3" style={{ flexWrap: "wrap" }}>
          <div className="field" style={{ marginBottom: 0 }}>
            <label htmlFor="runtime-usage-from" className="field-label" style={{ marginBottom: 4 }}>From</label>
            <input id="runtime-usage-from" aria-label="From date" className="prop-input" type="date" value={from} onChange={(e) => setFrom(e.target.value)} style={{ width: 160 }} />
          </div>
          <div className="field" style={{ marginBottom: 0 }}>
            <label htmlFor="runtime-usage-to" className="field-label" style={{ marginBottom: 4 }}>To</label>
            <input id="runtime-usage-to" aria-label="To date" className="prop-input" type="date" value={to} onChange={(e) => setTo(e.target.value)} style={{ width: 160 }} />
          </div>
