...
export const Route = createFileRoute("/runtimes/bindings")({
  ...
  ssr:false,
  component: RuntimeBindingsRoute,
});

function RuntimeBindingsRoute() {
  const pathname = useRouterState({ select: (s) => s.location.pathname }) as string;
  const isDetail = pathname !== "/runtimes/bindings" && pathname !== "/runtimes/bindings/" && pathname.startsWith("/runtimes/bindings/");
    ...
    <>
      <section className="mt-4">
        <div className="card-panel card-panel--elevated">
          <label htmlFor="runtime-project-select" className="field-label">Project</label>
            ...
            {projectsLoading ? (
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (pid) void navigate({ to: "/runtimes/bindings/$projectId", params: { projectId: pid } });
                }}
              >
