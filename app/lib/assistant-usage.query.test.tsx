...
describe("useAssistantUsage", () => {
  ...
  it("fetches usage and renders KPI cards", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining("/api/admin/assistant/usage"), expect.anything());
  });
  it("builds query with from/to/projectId", async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => mockUsage });
    vi.stubGlobal("fetch", fetchMock);
    function Probe() {
      const q = useAssistantUsage({ from: "2026-08-01", to: "2026-08-27", projectId: "p1" });
...
describe("usePutAssistantPrice", () => {
  ...
  it("merges the mutation response into the prices cache without invalidating usage", async () => {
    ...
    await waitFor(() => expect(qc.getQueryData(["assistant-prices"])).toEqual({ data: [updated] }));
