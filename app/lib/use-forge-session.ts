...
// conversation the next Forge run continues. Refetched on popover open so a
// background run that minted a session shows up.
export function useForgeSession(documentType: "task" | "wiki", documentId: string, enabled = true) {
  return useQuery({
    queryKey: ["forge-sessions", documentType, documentId],
    queryFn: () => api.listForgeSessions(documentType, documentId).then((r) => r.data),
    enabled,
  });
}
...
export function useResetForgeSession() {
