...
function useAssistantRun(args: PanelArgs & { prompt: string; effectiveSkillId: string; docImages: ReturnType<typeof toDocImages> }) {
  ...
  return { stream, running, done, failed, taskId, documentTitle: assistantTaskData?.documentTitle, selectionText: selection.text, generate, stop, dismiss, createPending: createTask.isPending };
}
