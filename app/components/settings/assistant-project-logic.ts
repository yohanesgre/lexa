...
export type ProviderLike = { id: string; label: string; baseUrl?: string | undefined; models?: AssistantProviderModel[] | undefined };

export type EnabledModel = AssistantProviderModel & { providerLabel: string; providerId: string; baseUrl: string };
...
export function enabledModelsOf(provider: ProviderLike | undefined): AssistantProviderModel[] {
...
export function fallbackRowsFor(fallbacks: string[], all: EnabledModel[]): Array<AssistantProviderModel & { providerLabel: string; providerId: string }> {
...
export async function testConnection(projectId: string, body: { providerId: string | null; modelId: string | null; fallbackModelIds: string[] }): Promise<ConnectionTestResult> {
  try {
    const res = await fetch(`/api/assistant/settings/${projectId}/test`, {
      method: "POST",
