...
import { useAssistantSettings, useSaveAssistantSettings } from "../../../lib/queries";
...
function storedBaseInput(settings: AssistantSettingsMasked): AssistantSettingsInput {
  return { kind: settings.kind, baseUrl: settings.baseUrl, model: settings.model } as AssistantSettingsInput;
}
...
export function AssistantEngineSection({ project }: { project: Project }) {
  const { data: settings } = useAssistantSettings(project.id);
  const save = useSaveAssistantSettings(project.id);
  const [engine, setEngine] = useState<RuntimeEngine>("assistant");
  ...
  const persist = (patch: Partial<AssistantSettingsInput>) => {
    if (!settings) return;
  ...
  return (
    <section className="mb-8">
      ...
      <p className="text-sm text-lx-text-secondary mb-4" style={{ maxWidth: 640 }}>
        Which execution tier document threads and Generate use for this project: Assistant (server-side writing assistant) or Blacksmith (daemon coding agent). Freeform chat always runs the Assistant lane.
      </p>
      ...
      <div className="card-panel card-panel--elevated">
        <div className="field">
          ...
          >
            <button type="button" role="radio" aria-checked={engine === "assistant"} className="btn btn-sm" style={optionStyle(engine === "assistant")} onClick={() => { setEngine("assistant"); persist({ engine: "assistant" }); }}>
        ...
        <div className="field">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm text-lx-text-secondary">Members get a personal Assistant | Blacksmith toggle in the AI popover header</span>
          </div>
        ...
        <div className="text-xs text-lx-text-secondary" style={{ border: "1px solid var(--lx-border-default)", borderRadius: 6, padding: "10px 12px", maxWidth: 640, lineHeight: "18px" }}>
          <strong className="font-medium text-lx-text-primary">Chat gate.</strong> Freeform chat ALWAYS runs the Assistant lane regardless of this setting; under engine=&apos;blacksmith&apos; chat streams fail with <span className="font-mono">ENGINE_NOT_SUPPORTED_FOR_CHAT</span> (409).
        </div>
      </div>
    </section>
  );
