...
import { useAssistantMemory, useAddAssistantMemory, useRemoveAssistantMemory } from "../../../lib/queries";
import type { AssistantMemoryEntry } from "../../../lib/api";
      ...
      <div className="card-panel card-panel--elevated">
        {isLoading ? (
          ...
          <div className="empty-box" style={{ padding: "20px 16px" }}>
            ...
            <p className="text-xs text-lx-text-secondary" style={{ maxWidth: 360 }}>Add project conventions Assistant should respect in every run. Leave empty to run without injected memory.</p>
          </div>
        ) : (
...
function MemoryRow({ memory, onDelete, deleting }: { memory: AssistantMemoryEntry; onDelete: () => void; deleting: boolean }) {
  const isAssistant = memory.source === "assistant";
  return (
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className="font-micro text-2xs text-lx-text-muted uppercase tracking-[0.04em]">
            {isAssistant ? `saved by Assistant · ${formatRelative(memory.updatedAt)}` : `updated ${formatRelative(memory.updatedAt)}`}
          </span>
        </div>
      </div>
