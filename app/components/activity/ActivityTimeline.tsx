...
function EventRow({ item, ownerName }: { item: ActivityEvent; ownerName: string | null }) {
  ...
  // Provenance pill (assistant-write-approvals.html State 5): marks rows WRITTEN
  // THROUGH an approved Assistant write tool. Read defensively — viaAssistant rides
  // the API payload only when the backend serializes it.
  const viaAssistant = (item as { viaAssistant?: unknown }).viaAssistant === true;
  return (
    <div className="activity-row" style={{ "--marker-center": "16px" } as CSSProperties}>
      ...
      <div className="activity-body">
        ...
        <div style={isAgent ? { marginTop: 2 } : undefined}>
          ...
          {viaAssistant && <span className="via-pill" style={{ marginLeft: 6 }}>via Assistant</span>}
