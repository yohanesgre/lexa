...
const AGENTS: Array<{ id: HearthProvider; name: string; desc: string; runs: string }> = [
...
function StepAgent({ agentCli, onSelect, onBack, onNext }: {
  agentCli: HearthProvider | null;
  onSelect: (id: HearthProvider) => void;
  onBack: () => void;
  onNext: () => void;
}) {
  return (
  ...
  return (
    <div>
      ...
      <Field label="Key name" htmlFor="runtime-key-name" className="mb-4">
        <div className="flex gap-2">
          <TextInput id="runtime-key-name" value={newKeyName} onChange={onKeyNameChange} placeholder="e.g. hearth-opencode" className="flex-1" onKeyDown={(event) => { if (event.key === "Enter") onCreateKey(); }} />
...
export function RuntimeSetupModal({ onClose }: { onClose: () => void }) {
  ...
  const [agentCli, setAgentCli] = useState<HearthProvider | null>(null);
