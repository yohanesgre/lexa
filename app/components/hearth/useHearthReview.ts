...
const HEARTH_APPLIED_KEY = "lxk.hearth-applied-task";
const HEARTH_REJECTED_KEY = "lxk.hearth-rejected-task";

export interface HearthReviewIdentity {
  action: string;
...
export interface HearthReviewState {
  result: string;
  action: string;
  runtime: string | null;
  taskId: string | null;
...
export function useHearthReview(editor: Editor | null, onReviewStateChange?: (active: boolean, accepted: boolean) => void) {
  const [review, setReview] = useState<HearthReviewState | null>(null);
  // Accepted hearth result — terminal for this tab session, so a reload
  // doesn't re-offer it for insert (duplicate risk).
  const [appliedTaskId, setAppliedTaskId] = useState<string | null>(() =>
    typeof window !== "undefined" ? sessionStorage.getItem(HEARTH_APPLIED_KEY) : null
  );
  // Rejected hearth result — same terminal semantics: the popover re-attaches
  ...
  const [rejectedTaskId, setRejectedTaskId] = useState<string | null>(() =>
    typeof window !== "undefined" ? sessionStorage.getItem(HEARTH_REJECTED_KEY) : null
  );

  const handleReview = (text: string, identity: HearthReviewIdentity) => {
  ...
  const handleAcceptReview = () => {
    ...
    if (review.taskId) sessionStorage.setItem(HEARTH_APPLIED_KEY, review.taskId);
    setReview(null);
  };
  ...
  const handleRejectReview = () => {
    ...
    if (review.taskId) sessionStorage.setItem(HEARTH_REJECTED_KEY, review.taskId);
  };
