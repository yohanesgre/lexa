...
export function DeleteSwimlaneDialog({ target, taskCount, onClose, onDelete }: {
  target: Swimlane;
  taskCount?: number | undefined;
  onClose: () => void;
  onDelete: () => void;
}) {
  const copy = taskCount === undefined
    ? "This action cannot be undone."
    : taskCount > 0
      ? `This swimlane has ${taskCount} ${taskCount === 1 ? "task" : "tasks"}. Reassign them to another swimlane first — a swimlane with tasks can't be deleted.`
      : "This swimlane has no tasks. This action cannot be undone.";
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[80] pointer-events-none">
        <dialog open className="dialog dialog-enter pointer-events-auto" aria-modal="true" aria-label="Confirm">
          ...
          <p className="text-sm text-lx-text-secondary mt-3 leading-5">{copy}</p>
