...
// personal overlay (localStorage runtime-engine-overlay:<projectId>) wins
// only when the project enabled the switcher. It never writes settings.
function changeModeFor(projectId: string | undefined, next: RuntimeMode): RuntimeMode {
...
function resolveSkillState(agents: { id: string; skillIds: string[] }[], skills: LexaSkill[], mode: RuntimeMode, skillId: string) {
...
function HeaderRight({ done, failed, running, switcherEnabled, mode, changeMode, taskRunning }: {
  done: boolean; failed: boolean; running: boolean; switcherEnabled: boolean; mode: RuntimeMode; changeMode: (next: RuntimeMode) => void; taskRunning: boolean;
}) {
...
// fresh for the next Runtime run.
...
function buildCreateTaskInput(slug: string, documentType: "task" | "wiki", documentId: string, mode: RuntimeMode, skillId: string, extraPrompt: string, selection: string, runtimeId: string) {
  return {
    slug,
    documentType,
    documentId,
...
function HeraldPortalView({ containerRef, popoverStyle, editor, slug, documentType, documentId, switcherEnabled, changeMode, onClose, onReview, reviewActive, appliedTaskId, rejectedTaskId, portalTarget }: {
  ...
  changeMode: (next: RuntimeMode) => void;
  onClose: () => void;
  onReview: (text: string, identity: { action: string; runtimeName: string | null; provider: string | null; taskId: string }) => void;
  reviewActive: boolean;
  appliedTaskId: string | null | undefined;
...
function PopoverBody({ taskId, taskData, running, failed, done, reviewActive, followLog, setFollowLog, logBodyRef, logs, isAdmin, setLogModalOpen, cancelTask, setTaskId, runtimes, onReview, agentSkills, effectiveSkillId, setSkillId, extraPrompt, setExtraPrompt, runtimeId, setRuntimeId, onlineRuntimes, sessionRow, resetSession, documentType, documentId, taskRunning, selectionText, onGenerate, creating, logModalOpen }: {
  ...
  logs: { data?: RuntimeTaskLog[] | undefined };
  isAdmin: boolean;
...
export function RuntimePopover({ editor, slug, documentType, documentId, open, onClose, onReview, reviewActive, appliedTaskId, rejectedTaskId, anchorRect }: RuntimePopoverProps) {
  ...
  const changeMode = (next: RuntimeMode) => {
    ...
    // engine agent (runtime-popover.html annotations).
    setSkillId("");
  };
  ...
  // Any online runtime can run tasks — Runtime uses the daemon's agent CLI
  ...
  const createTask = useCreateRuntimeTask();
  ...
  const sessions = useRuntimeSession(documentType, documentId, open && mode === "blacksmith");
  const resetSession = useResetRuntimeSession();
  const attached = open && taskId !== null;
  const task = useRuntimeTask(taskId, attached);
  ...
  const logs = useRuntimeTaskLogs(taskId, attached && isAdmin);
  ...
  const recent = useRecentRuntimeTask(slug, documentType, documentId, open && taskId === null);
