...
const RUNTIME_APPLIED_KEY = "lxk.runtime-applied-task";
const RUNTIME_REJECTED_KEY = "lxk.runtime-rejected-task";

export interface RuntimeReviewIdentity {
  action: string;
...
export interface RuntimeReviewState {
  result: string;
  action: string;
  runtime: string | null;
  taskId: string | null;
...
export function useRuntimeReview(editor: Editor | null, onReviewStateChange?: (active: boolean, accepted: boolean) => void) {
  const [review, setReview] = useState<RuntimeReviewState | null>(null);
  // Accepted runtime result — terminal for this tab session, so a reload
  // doesn't re-offer it for insert (duplicate risk).
  const [appliedTaskId, setAppliedTaskId] = useState<string | null>(() =>
    typeof window !== "undefined" ? sessionStorage.getItem(RUNTIME_APPLIED_KEY) : null
  );
  // Rejected runtime result — same terminal semantics: the popover re-attaches
  ...
  const [rejectedTaskId, setRejectedTaskId] = useState<string | null>(() =>
    typeof window !== "undefined" ? sessionStorage.getItem(RUNTIME_REJECTED_KEY) : null
  );

  const handleReview = (text: string, identity: RuntimeReviewIdentity) => {
  ...
  const handleAcceptReview = () => {
    ...
    if (review.taskId) sessionStorage.setItem(RUNTIME_APPLIED_KEY, review.taskId);
    setReview(null);
  };
  ...
  const handleRejectReview = () => {
    ...
    if (review.taskId) sessionStorage.setItem(RUNTIME_REJECTED_KEY, review.taskId);
  };
