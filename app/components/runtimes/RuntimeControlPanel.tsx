...
import { useCancelRuntimeTask, useRuntimeTask, useRuntimeTaskHistory, useRuntimeTaskLogs, useSkills, useProjects, useRuntimes, useSession } from "../../lib/queries";
...
function durationLabel(task: RuntimeTask): string {
...
function timelineLabel(task: RuntimeTask): string {
...
function openDocumentPath(task: RuntimeTask, projectSlug: string | undefined): string {
...
function emptyStateTitle(status: RuntimeTaskStatus | null, slug: string, skillId: string, teamId: string, cursor: string | null): string {
...
function emptyStateHint(status: RuntimeTaskStatus | null, slug: string, skillId: string, teamId: string, cursor: string | null): string {
  const filtered = status !== null || slug !== "" || skillId !== "" || teamId !== "";
...
function SummaryStrip({ summary, activeCount, online, total }: {
  summary: Record<RuntimeTaskStatus, number> | undefined;
  activeCount: number;
  online: number;
  total: number;
}) {
...
interface RuntimeTaskRow extends RuntimeTask {
  projectName?: string;
}
...
function DetailCrumb({ detail }: { detail: RuntimeTaskRow | null }) {
  const label = detail
  ...
  return (
    <div className="runtime-log">
      <div className="runtime-log-head">
        <span className={cn("runtime-log-live", !logActive && "is-static")}>
      ...
      {lines.length === 0 ? (
        ...
        <div className="runtime-log-body">
          {lines.map((line, i) => {
            ...
            return (
              <div key={line.id} className={cn("runtime-log-line", level === "error" && "stderr", level === "warn" && "warn", logActive && isLast && "current")}>
                <span className="runtime-log-dot" aria-hidden="true">{level === "info" ? "●" : "!"}</span>
                <span className="runtime-log-time">{formatLogTime(line.createdAt)}</span>
                <span className="runtime-log-msg">{display}</span>
              </div>
            );
          })}
        </div>
    ...
    <>
      ...
      {detail.status === "completed" && (
        <>
          ...
          <div className="runtime-result-card runtime-result-md">
...
export function HistoryStates({ history, page, filters, cursor, children }: {
  history: UseQueryResult<{ data: RuntimeTaskRow[]; summary?: Record<RuntimeTaskStatus, number>; nextCursor?: string | null }, unknown>;
  page: RuntimeTaskRow[];
...
function useRuntimeRunsData(filters: { slug: string; status: RuntimeTaskStatus | null; skillId: string; teamId: string }, cursor: string | null, selectedId: string | null) {
  const skills = useSkills();
  const history = useRuntimeTaskHistory(
    {
  ...
  const selected = useRuntimeTask(selectedId, selectedId !== null);
  // Log internals are ADMIN-GATED — members never fetch the feed (403).
  const { data: session } = useSession();
  const isAdmin = session?.user?.role === "superadmin";
  const logs = useRuntimeTaskLogs(selectedId, selectedId !== null && isAdmin);
  const cancelTask = useCancelRuntimeTask();
  return { skills, history, runtimes, projects, selected, isAdmin, logs, cancelTask };
}

export function RuntimeControlPanel({ embedded = false }: { embedded?: boolean }) {
  const portalTarget = typeof document !== "undefined" ? document.body : null;
  // ?task=<id> deep-link (navbar Runtime dropdown rows) — opens the record.
  const search = useSearch({ from: "/runtimes/runs" });
  const [status, setStatus] = useState<RuntimeTaskStatus | null>(null);
  ...
  const { skills, history, runtimes, projects, selected, isAdmin, logs, cancelTask } = useRuntimeRunsData({ slug, status, skillId, teamId }, cursor, selectedId);
  const { isSuperadmin, teams } = useRuntimeRole();
  ...
  const reset = (next: { slug?: string | undefined; status?: RuntimeTaskStatus | null; skillId?: string; teamId?: string }) => {
  ...
  const detail = (selected.data ?? row ?? null) as RuntimeTaskRow | null;
    ...
    <>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3">
          <Link to="/runtimes/daemons" className="btn btn-ghost" style={{ height: 28, padding: "0 12px", fontSize: 12, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>
...
export function RuntimeRunsContent() {
  return <RuntimeControlPanel embedded />;
}
