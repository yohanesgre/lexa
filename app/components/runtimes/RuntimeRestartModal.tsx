...
function OfflineCommand({ machine }: { machine: Machine | undefined }) {
  ...
  const copy = () => {
    void copyToClipboard("lx machine listen").then(() => {
      setCopied(true);
  ...
  return (
    <div className="field">
      ...
      <div style={{ background: "var(--lx-surface-input)", border: "1px solid var(--lx-border-default)", borderRadius: 6, padding: 12, position: "relative" }}>
        <pre className="font-mono text-xs text-lx-text-secondary whitespace-pre-wrap leading-6 m-0">lx machine listen</pre>
...
function SystemdCommand() {
  return (
    <div className="field">
      ...
      <pre className="font-mono text-xs text-lx-text-secondary whitespace-pre-wrap leading-6 m-0" style={{ background: "var(--lx-surface-input)", border: "1px solid var(--lx-border-default)", borderRadius: 6, padding: 12 }}>{`systemctl --user restart lexa-machine-listener
journalctl --user -u lexa-machine-listener -f   # watch it reconnect`}</pre>
        ...
        <dialog open className="dialog dialog-enter pointer-events-auto" aria-modal="true" aria-label="Restart runtime" style={{ maxWidth: 560, width: "100%" }}>
          ...
          <div className="modal-body">
            ...
            <RestartFooter
              ...
              onClose={onClose}
              onRestart={restart}
              onCheckAgain={() => void qc.refetchQueries({ queryKey: ["machines"] })}
            />
          </div>
        </dialog>
      </div>
