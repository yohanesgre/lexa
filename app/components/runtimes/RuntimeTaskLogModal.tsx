...
const STATUS_META: Record<RuntimeTaskStatus, { label: string; color: string }> = {
...
function logToText(lines: RuntimeTaskLog[]): string {
...
function LogLineRow({ line, active, isLast }: { line: RuntimeTaskLog; active: boolean; isLast: boolean }) {
  const { level, display } = classifyLogLine(line);
  return (
    <div className={cn("runtime-log-line", level === "error" && "stderr", level === "warn" && "warn", active && isLast && "current")}>
...
function FollowToggle({ followLog, onToggle }: { followLog: boolean; onToggle: () => void }) {
  return (
    <>
      ...
      <span className="runtime-log-live" style={{ marginLeft: 8 }}>live</span>
    </>
  );
}

function LogBody({ lines, status }: { lines: RuntimeTaskLog[]; status: RuntimeTaskStatus }) {
  if (lines.length === 0) {
    return (
...
// Expanded log viewer for a Runtime task — same append-only feed as the compact
// .runtime-log, but full-height with wrapped lines, follow + copy. Shared
// by the editor popover and the control panel slideover (runtime-log-modal).
export function RuntimeTaskLogModal({
  open,
  onClose,
  task,
  logs,
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
        <dialog open className="dialog dialog-enter runtime-log-modal pointer-events-auto" aria-modal="true" aria-label="AI task log">
          ...
          <div className="runtime-log-modal-body">
            ...
            <div className="runtime-log runtime-log-expanded" ref={bodyRef}>
