...
import type { AgentCli, Machine, Team } from "../../../shared/types";

const STEPS = ["Machine", "Agent CLI", "Key / Send", "Verify"] as const;

const AGENTS: Array<{ id: AgentCli; name: string; desc: string; runs: string }> = [
  ...
  return (
    <div>
      ...
      <Field label="Key name" htmlFor="runtime-key-name" className="mb-4">
        <div className="flex gap-2">
          <TextInput id="runtime-key-name" value={newKeyName} onChange={onKeyNameChange} placeholder="e.g. runtime-opencode" className="flex-1" onKeyDown={(event) => { if (event.key === "Enter") onCreateKey(); }} />
...
export function RuntimeSetupModal({ onClose }: { onClose: () => void }) {
  ...
  const [agentCli, setAgentCli] = useState<AgentCli | null>(null);
