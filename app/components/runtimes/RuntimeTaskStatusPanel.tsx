...
const STATUS_LABEL: Record<RuntimeTaskStatus, string> = {
  queued: "Queued",
  running: "Running",
  completed: "Completed",
  failed: "Failed",
...
const STATUS_COLOR: Record<RuntimeTaskStatus, string> = {
...
// Wireframe runtime-popover.html:238-266 — member-visible brief. One row per
...
export function TaskBrief({ taskData }: { taskData: RuntimeTask }) {
  const rows: Array<{ status: RuntimeTaskStatus; at: string }> = [{ status: "queued", at: taskData.createdAt }];
...
function TaskRunPanel({ taskData, running, followLog, setFollowLog, logBodyRef, logs, canViewLogs, setLogModalOpen, dismissedIdsRef, cancelTask, setTaskId, runtimes }: {
  ...
  return (
    <div style={{ padding: "0 12px 12px" }}>
      ...
      {running && canViewLogs && (
        <div className="runtime-log" style={{ marginBottom: 8 }}>
          <div className="slideover-body" style={{ padding: 0 }}>
            ...
            <div className="runtime-log">
              <div className="runtime-log-head">
                <span className="runtime-log-live">Live · {logLines.length} {logLines.length === 1 ? "line" : "lines"}</span>
              ...
              {logLines.length === 0 ? (
                <div className="runtime-log-empty">{taskData?.runtimeId ? "Waiting for daemon activity." : "Queued — waiting for a runtime to claim it."}</div>
              ) : (
                <div className="runtime-log-body" ref={logBodyRef}>
                  {logLines.map((line, index) => {
                    const { level, display } = classifyLogLine(line);
                    const isLast = index === logLines.length - 1;
                    return (
                      <div key={line.id} className={cn("runtime-log-line", level === "error" && "stderr", level === "warn" && "warn", isLast && "current")}>
