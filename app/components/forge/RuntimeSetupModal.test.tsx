...
beforeEach(() => {
  ...
  routes.clear();
  mockFetch();
  routes.set("GET /api/forge/machines", { data: [listeningMachine(), OFFLINE_MACHINE] });
...
describe("RuntimeSetupModal", () => {
  ...
  it("walks the full setup flow: machine → agent CLI → key → send install event", async () => {
    const user = userEvent.setup();
    routes.set("POST /api/settings/api-keys", {
      key: { id: "k1", name: "forge-opencode", createdAt: "t", lastUsedAt: null },
      rawKey: "lxk_secret123",
    });
    routes.set("POST /api/forge/runtime-events", { id: "ev1" });
    routes.set("GET /api/forge/runtime-events/ev1", { id: "ev1", status: "completed" });
    routes.set("GET /api/forge/runtimes", { data: [{ id: "r1", machineId: "m1", provider: "opencode", status: "online" }] });
    render(<RuntimeSetupModal onClose={() => {}} />, { wrapper });
    ...
    expect(screen.getByText("Create key and install")).toBeInTheDocument();
    await user.type(screen.getByLabelText("Key name"), "forge-opencode");
    await user.click(screen.getByRole("button", { name: "Create" }));
  ...
  it("surfaces a send failure in the step-2 error notice", async () => {
    ...
    routes.set("POST /api/forge/runtime-events", "HTTP_500");
    render(<RuntimeSetupModal onClose={() => {}} />, { wrapper });
