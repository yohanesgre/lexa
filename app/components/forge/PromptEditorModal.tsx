...
export function PromptEditorModal({ kind, entity, allSkills = [], allAgents = [], onClose }: PromptEditorModalProps) {
  ...
  const createAgent = useCreateForgeAgent();
  const updateAgent = useUpdateForgeAgent();
  const deleteAgent = useDeleteForgeAgent();
  const replaceSkills = useReplaceAgentSkills();
  const resetAgent = useResetForgeAgent();
  const createSkill = useCreateForgeSkill();
  const updateSkill = useUpdateForgeSkill();
  const deleteSkill = useDeleteForgeSkill();
