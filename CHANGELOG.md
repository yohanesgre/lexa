...
- **Forge/CLI hardening (dual oracle audit)** — machine identity bound: per-machine secret minted once at register, required on runtime-event claim (`x-machine-secret`; `machines.secret` via migration 0003; legacy machines re-register; claim 403 is oracle-free — no existence leak); cross-host re-register → 409 `MACHINE_ID_TAKEN`; `runtime_events` pruned (7-day, boot + hourly); forge task `result`/`error` capped (1 MB / 2 KB); rate limiter restored for `/api/setup*` + `/api/health`, exempt for `/api/forge/daemon/*` + `/api/forge/runtimes/register` (token-gated, log streams must not 429); agent child env whitelisted (no `LXK_*`/`LEXA_*`/`FORGE_*` inheritance — hermes/command-code run unsandboxed); listener kills daemon trees via /proc walk (no more orphaned agents on stop/delete); `daemon.pid` persisted + stale-daemon sweep at boot (no duplicate daemons after crashes); agent stdout/stderr bounded (1 MB / 64 KB tails + totals); `lexa-cli` SKILL.md corrected to real machine/listener commands
