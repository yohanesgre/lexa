...
- Generic skills — `goal`, `work-plans`, `git-workflow` — are global
  (`~/.agents/skills/`); this file's project rules win where they conflict.
...
Trunk `main` — never commit directly. Every task: `main` → new branch → work → PR → merge to `main`. Before any git mutation (branch/worktree, checkout -b, add, commit, push, merge/rebase/cherry-pick, tag `v*`/`cli-v*`, or `wireframes/` submodule) load the `git-workflow` skill and follow guardrails verbatim. No commit/push/merge without explicit user ask; gate (`tsc --noEmit` + `vitest run` + `check:invariants` + secrets check) must be green. Quick gate: `bash scripts/verify-gate.sh`.
...
All work tracking uses one folder per plan under `status/` (the global `work-plans` skill; load it when opening, updating, or closing a plan):
...
**Submodule commits (non-negotiable):** commit and push inside `wireframes/` first, then bump the pointer in the parent (see the `git-workflow` skill §6).
...
| 7 | Task and wiki document payloads speak TipTap JSON; AI runtime/assistant output may carry Markdown. Markdown conversion lives only in `shared/markdown.ts` (a shared pure module). The app may import it directly. | Single conversion surface. |
...
| 12 | Emission invariant. Every task mutation except deletion appends `task_activity` row(s) in the SAME transaction — one row per meaningful change (updates may emit several `field_changed` rows); position-only reorders emit nothing; webhook moves emit `github_synced` only. Task delete emits no activity row (`task_activity` rows cascade with the task). Messages come from the catalog (`server/activity-messages.ts`), frozen at write time — never hand-rolled at call sites. | Audit log + copy consistency. |
