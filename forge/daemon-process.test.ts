...
describe("forge daemon (bun subprocess) with a fake opencode serve runtime", () => {
  ...
  async function startFakeLexa(): Promise<void> {
    ...
    fakeLexa = createServer(async (req, res) => {
        ...
        if (!claimed) {
          ...
          res.end(JSON.stringify({
            task: {
              ...
              agentId: "lexa", agentName: "Lexa", skillId: "forge", skillName: "Forge",
      ...
      if (url === "/api/forge/sessions" && req.method === "PUT") {
        sessionsPuts.push({ body: JSON.parse(body) as Record<string, unknown> });
        res.writeHead(204);
        res.end();
        return;
      }
      if (url === "/api/forge/sessions" && req.method === "DELETE") {
        sessionsDeletes.push({ body: JSON.parse(body) as Record<string, unknown> });
        res.writeHead(204);
        res.end();
        return;
      }
      if (url.startsWith("/api/forge/daemon/tasks/") && url.endsWith("/status")) {
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ status: "cancelled" }));
        return;
      }
      if (url.startsWith("/api/forge/daemon/tasks/") && url.endsWith("/complete")) {
        completes.push({ url, body: JSON.parse(body) as Record<string, unknown> });
        res.end(JSON.stringify({ ok: true }));
        return;
      }
      if (url.startsWith("/api/forge/daemon/tasks/") && url.endsWith("/fail")) {
  ...
  function spawnDaemon(): ReturnType<typeof spawn> {
    return spawn("bun", ["forge/daemon.ts"], {
      ...
      env: {
        ...
        FORGE_MACHINE_ID: "m-test",
        FORGE_POLL_MS: "100",
        FORGE_SERVE_PORT: String(servePort),
        LEXA_DIR: dir,
        HOME: home,
      },
    });
